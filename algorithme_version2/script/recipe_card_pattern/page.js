...
function displayRecipes(tab) {
    ...
    tab.forEach((tab_element) => {
        let id = new Recipe(tab_element).id;
        let recipeIngredients = new Recipe(tab_element).ingredients;

        recipeIngredients.forEach((recipeIngredients_element) => {
            document.getElementById(id.toString()).innerHTML += displayIngredients(recipeIngredients_element);
        });
    });
